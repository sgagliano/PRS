#Hard-coded for InPSYght data

suppressPackageStartupMessages(require(ggplot2))

y<-read.table("../output/noref_inpsyght.splitvalidate.results.txt", as.is=T,h=T) #lassosum output
admix<-read.table("/net/inpsyght/disk2/sarahgag/PRS/admixture/scripts/InPSYght-4Admixture.5.Q-withIDs", as.is=T, h=T) #ADMIXTURE output
merge<-merge(y, admix, by.x="IID", by.y="IID")
x<-read.table("../../prs/input/InPSYghtcasectrl-withSexWave.txt", as.is=T, h=T) #phenotype
...
pdf(file="noref_AFR_plot.pdf",height=5,width=5,useDingbats=FALSE)
